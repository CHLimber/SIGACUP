...
return;
    ...
    if (paymentIntent && paymentIntent.status === 'succeeded') {
        try {
            const resp = await fetch(`/matricula/${props.token}/confirmar`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
            ...
            // Navegación de página completa para que el comprobante (vista Blade)
            // ocupe toda la ventana en lugar de abrirse en un modal de Inertia.
            window.location.href = data.redirect ?? `/matricula/${props.token}/comprobante`;
        } catch {
            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-2xl">
                ...
                <div class="border-b border-gray-100 px-6 py-5">
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        1ra opción: <strong>{{ candidato.carrera }}</strong>
                    </p>
                    <p v-if="candidato.carrera2" class="text-xs text-gray-500">
                        2da opción: <strong>{{ candidato.carrera2 }}</strong>
                    </p>
                </div>
