...
interface DatosAcademicos {
    carrera1_id: number | null;
    carrera2_id: number | null;
...
const faltaDatosAcademicos = computed(() =>
    props.candidato.tipo === 'estudiante'
    && (!props.datosAcademicos?.carrera1_id
        || !props.datosAcademicos?.carrera2_id
        || !props.datosAcademicos?.anio_egreso
        || !props.datosAcademicos?.unidad_educativa
        || !props.datosAcademicos?.tipo_colegio),
);
...
const academicoForm = useForm({
    carrera1_id:      props.datosAcademicos?.carrera1_id ?? null,
    carrera2_id:      props.datosAcademicos?.carrera2_id ?? null,
    anio_egreso:      props.datosAcademicos?.anio_egreso ?? null,
    unidad_educativa: props.datosAcademicos?.unidad_educativa ?? '',
    tipo_colegio:     props.datosAcademicos?.tipo_colegio ?? '',
});

const anioActual = new Date().getFullYear();

function puedeEditarDatosAcademicos(): boolean {
    return props.candidato.tipo === 'estudiante'
        && props.candidato.estado !== 'en_revision'
        && !props.bloqueado;
}

function guardarDatosAcademicos() {
    academicoForm.post(`/candidato/${props.token}/datos-academicos`, {
        preserveScroll: true,
    });
}
...
<template>
    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <main class="mx-auto max-w-4xl px-6 py-8">
            ...
            <!-- Datos académicos (solo estudiantes) -->
            <div
                v-if="candidato.tipo === 'estudiante' && datosAcademicos"
                class="mt-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm"
            >
                <div class="flex items-start gap-3">
                    <School class="h-6 w-6 flex-shrink-0 text-[#c70e0a]" />
                    <div class="flex-1">
                        <h2 class="text-base font-semibold text-gray-900">Datos académicos</h2>
                        <p class="mt-0.5 text-sm text-gray-500">
                            Indica las dos carreras a las que postulas y los datos de tu colegio. Se usarán para tu admisión.
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <!-- 1ra opción -->
                    <div>
                        <label class="block text-xs font-semibold text-gray-700">1ra opción de carrera *</label>
                        <select
                            v-model="academicoForm.carrera1_id"
                            :disabled="!puedeEditarDatosAcademicos()"
                            class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#c70e0a] focus:outline-none focus:ring-1 focus:ring-[#c70e0a] disabled:bg-gray-50 disabled:text-gray-500"
                        >
                            ...
                            <option
                                ...
                                :key="c.id"
                                :value="c.id"
                                :disabled="c.id === academicoForm.carrera2_id"
                            >{{ c.nombre }}</option>
                        </select>
                        <p v-if="academicoForm.errors.carrera1_id" class="mt-1 text-xs text-red-600">{{ academicoForm.errors.carrera1_id }}</p>
                    </div>

                    <!-- 2da opción -->
                    <div>
                        <label class="block text-xs font-semibold text-gray-700">2da opción de carrera *</label>
                        <select
                            v-model="academicoForm.carrera2_id"
                            :disabled="!puedeEditarDatosAcademicos()"
                            class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#c70e0a] focus:outline-none focus:ring-1 focus:ring-[#c70e0a] disabled:bg-gray-50 disabled:text-gray-500"
                        >
                            <option :value="null" disabled>Selecciona una carrera</option>
                            <option
                                ...
                                :key="c.id"
                                :value="c.id"
                                :disabled="c.id === academicoForm.carrera1_id"
                            >{{ c.nombre }}</option>
                        </select>
                        <p v-if="academicoForm.errors.carrera2_id" class="mt-1 text-xs text-red-600">{{ academicoForm.errors.carrera2_id }}</p>
                    </div>

                    <!-- Unidad educativa -->
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-gray-700">Unidad educativa (colegio) *</label>
                        <input
                            v-model="academicoForm.unidad_educativa"
                            type="text"
                            maxlength="150"
                            ...
                            class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#c70e0a] focus:outline-none focus:ring-1 focus:ring-[#c70e0a] disabled:bg-gray-50 disabled:text-gray-500"
                        />
                        <p v-if="academicoForm.errors.unidad_educativa" class="mt-1 text-xs text-red-600">{{ academicoForm.errors.unidad_educativa }}</p>
                    </div>

                    <!-- Año de egreso -->
                    <div>
                        <label class="block text-xs font-semibold text-gray-700">Año de egreso *</label>
                        <input
                            v-model.number="academicoForm.anio_egreso"
                            type="number"
                            min="1980"
                            :max="anioActual"
                            placeholder="ej. 2024"
                            ...
                            class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#c70e0a] focus:outline-none focus:ring-1 focus:ring-[#c70e0a] disabled:bg-gray-50 disabled:text-gray-500"
                        />
                        <p v-if="academicoForm.errors.anio_egreso" class="mt-1 text-xs text-red-600">{{ academicoForm.errors.anio_egreso }}</p>
                    </div>

                    <!-- Tipo de colegio -->
                    <div>
                        <label class="block text-xs font-semibold text-gray-700">Tipo de colegio *</label>
                        <select
                            v-model="academicoForm.tipo_colegio"
                            :disabled="!puedeEditarDatosAcademicos()"
                            class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-[#c70e0a] focus:outline-none focus:ring-1 focus:ring-[#c70e0a] disabled:bg-gray-50 disabled:text-gray-500"
                        >
                        ...
                        <p v-if="academicoForm.errors.tipo_colegio" class="mt-1 text-xs text-red-600">{{ academicoForm.errors.tipo_colegio }}</p>
                    </div>
                </div>

                <div v-if="puedeEditarDatosAcademicos()" class="mt-4 flex justify-end">
                    <button
                        type="button"
                        class="rounded-md bg-[#c70e0a] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#a00b08] disabled:opacity-50"
                        :disabled="academicoForm.processing"
                        @click="guardarDatosAcademicos"
                    >
                        {{ academicoForm.processing ? 'Guardando…' : 'Guardar datos académicos' }}
                    </button>
                </div>
            </div>
